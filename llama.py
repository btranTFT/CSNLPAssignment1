...
class Attention(nn.Module):
    ...
    def compute_query_key_value_scores(self,
        ...
        causal_mask = torch.triu(
            torch.full((seqlen, seqlen), float("-inf"), device=query.device),
            diagonal=1,
        )
        scores = scores + causal_mask
        attention = F.softmax(scores.float(), dim=-1).type_as(query)
...
class LlamaLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        hidden_state = x + attention_output
        feed_forward_output = self.feed_forward(self.ffn_norm(hidden_state))
        return hidden_state + feed_forward_output
